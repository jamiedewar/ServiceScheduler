...
- In browser-only or static-file demo mode, the login control switches local demo roles so testers can see role-specific navigation without needing the backend API.
...
- The dispatch UI keeps the daily scheduling path visible by default and groups lower-frequency actions into Planning tools, Manual assignment and exceptions, Filters and saved views, and Admin / data panels.
